...
pub use self::{
    builder::RedisCacheBuilder,
    config::{Config, ResourceType},
    stats::RedisCacheStats,
};

#[cfg(feature = "permission-calculator")]
pub use self::permission::RedisCachePermissions;
...
/// reference to it is being held then calls to [`RedisCache::update`] may
/// be blocked.
///
...
// When adding a field here, be sure to add it to `RedisCache::clear` if
// necessary.
...
impl RedisCache {
    ...
    /// # Examples
    ///
    /// Creating a new `RedisCache` with a custom configuration, limiting
    /// the message cache to 50 messages per channel:
    ///
    /// ```
    /// use twilight_cache_redis::RedisCache;
    ///
    /// let cache = RedisCache::builder().message_cache_size(50).build();
    /// ```
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub const fn builder() -> RedisCacheBuilder {
    ...
    pub const fn iter(&self) -> RedisCacheIter<'_> {
    ...
    /// ```
    pub const fn stats(&self) -> RedisCacheStats<'_> {
    ...
    /// use twilight_cache_redis::{RedisCache, ResourceType};
    ...
    /// ```
    #[cfg(feature = "permission-calculator")]
    pub const fn permissions(&self) -> RedisCachePermissions<'_> {
        RedisCachePermissions::new(self)
    }
...
pub trait UpdateCache: private::Sealed {
    ...
    fn update(&self, cache: &RedisCache) {}
}
